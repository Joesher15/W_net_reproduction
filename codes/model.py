...
class ConvModule(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        layers = [
            ...
            # nn.Conv2d(input_dim, output_dim, 3, padding=1, groups=groups1),  # Depthwise (3x3) through each channel
...
class BaseNet(nn.Module):  # 1 U-net
    def __init__(self, input_channels=3,
        ...
        self.dec_modules = nn.ModuleList(
            [ConvModule(3 * channels_out, channels_out) for channels_out in decoder_out_sizes]) # (1536, 512), (768, 256), (384, 128)
    ...
    def forward(self, x):
        ...
        x_ = activations.pop(-1)  # popping out the last encoder module

        for conv, upconv in zip(self.dec_modules, self.dec_transpose_layers):
            skip_connection = activations.pop(-1)  # popping out the last encoder module from the remaining
            x_ = conv(
