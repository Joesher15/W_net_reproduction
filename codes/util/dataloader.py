...
def _get_img_list(folder):
    ...
    img_directory = os.path.join(folder, 'datasets/test_set/BSR/BSDS500/data/images/')
    gt_directory = os.path.join(folder, 'datasets/test_set/BSR/BSDS500/data/groundTruth/')
    pred_folder = os.path.join(folder, 'results/test_set_predictions/')
    path_folders=['val','train','test']
    for dir in path_folders:

        img_folder=os.path.join(img_directory,dir)
        gt_folder = os.path.join(gt_directory, dir)
