...
def _get_img_list(folder):
    ...
    img_directories = os.path.join(folder, 'datasets/test_set/BSR/BSDS500/data/images/')
    gt_directories = os.path.join(folder, 'datasets/test_set/BSR/BSDS500/data/groundTruth/')
    pred_folder = os.path.join(folder, 'results/test_set_predictions/')
    path_files = ['test/', 'train/', 'val/']
    for dir in path_files:
        img_folder=img_directories +dir
        gt_folder=gt_directories +dir
        # reading depth data
